package com.demo_backend.configuration.security;

import com.demo_backend.configuration.exception.ErrorHandler;
...
public class SecurityConfiguration {
    ...
    public SecurityFilterChain securityFilterChain(
        ...
        return http
                ...
                .exceptionHandling(configurer -> configurer    // này là ngoại lệ như ko logind đc hoặc ko có quyền
                        .authenticationEntryPoint(handler)
                        .accessDeniedHandler(handler)
                )
                .sessionManagement(configurer -> configurer   // ko lưu trạng thái
                        .sessionCreationPolicy(SessionCreationPolicy.STATELESS)
                )
                .addFilterBefore(jwtLoginFilter, UsernamePasswordAuthenticationFilter.class)
                .addFilterBefore(jwtAuthenticationFilter, UsernamePasswordAuthenticationFilter.class)
                .cors(withDefaults())   // cho phép quyền truy cập: vd từ cổng 5 -> 8
                .csrf(AbstractHttpConfigurer::disable)
                .formLogin(AbstractHttpConfigurer::disable)
                .logout(AbstractHttpConfigurer::disable)
                .rememberMe(AbstractHttpConfigurer::disable)
                .httpBasic(AbstractHttpConfigurer::disable)
                // mấy này là tắt mấy cái mặc định và dùng cái tự triển khai
                .build();
    }
